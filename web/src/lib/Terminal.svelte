        ...
        switch (message.type) {
          ...
          case 'AGENT_LAUNCHED':
            ...
            if (autoLaunchClaude && isReconnectingToExistingSession && terminal) {
              setTimeout(() => {
                console.log('[Terminal] Claude AGENT_LAUNCHED - sending space to trigger render');
                terminal.focus();
                // Send a space to trigger rendering
                if (ws && ws.readyState === WebSocket.OPEN) {
          ...
          case 'RECONNECTED':
            ...
            if (terminal) {
              setTimeout(() => {
                ...
                // For Claude, just type a space to trigger rendering
                if (autoLaunchClaude) {
                  terminal.focus();
                  // Just send a space to Claude - this triggers rendering and doesn't hurt anything
                  if (ws && ws.readyState === WebSocket.OPEN) {
                    ws.send(JSON.stringify({
                      type: 'SEND_INPUT',
                      payload: { input: ' ' }
  ...
  onMount(async () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.morphboxTerminals[panelId] = {
        ...
        clear,
        clearSession,
        cliType: autoLaunchClaude ? 'claude' : 'bash', // Default CLI type
        getBufferContent: () => {
          if (!terminal) return '';
          const lines = [];
