...
// Create global morphbox command - now just a symlink to the npm-installed command
function createGlobalCommand(morphboxHome) {
  // The morphbox command is now installed via npm, so we just need to check if it's accessible
  try {
    execSync('which morphbox', { stdio: 'ignore' });
    log.success('morphbox command is already available globally');
  } catch (e) {
    log.info('morphbox command will be available after npm install completes');
  }
